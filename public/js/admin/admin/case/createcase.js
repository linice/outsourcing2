...
//		}
	...
	function init() {
		...
		$("#btn_save").click(function() {
			if (valForm()) {
				$.ajax({
					url: '/admin/admin_case/savecaseindraft',
					type: 'post',
					async: false,
					...
					success: function(data, textStatus, jqXHR) {
						if (data['err'] == 0) { //用户注册信息通过验证
							if (!!data["id"]) {
								$("#type").val(data["type"]);
								$("#caseId").val(data["id"]);
								$("#searchLpBtn").attr("disabled", true);
							}
							alert(data['msg']);
						} else { //用户注册信息不合法
		...
		$('#btn_confirm').click(function() {
			if (valForm()) {
				//$('#form_case').attr("action", "/admin/admin_case/createcaseconfirm");
				$('#form_case').attr("action", "/admin/admin_case/publishcase");
				$('#form_case').submit();
			}
		})
		...
		$('#akbSettingBtn').click(function(){
			...
			$('#dlg_recmdEmp').dialog({
				...
				width: 800,
				open: function() {
					$("#dlgmoney").text($("#dlgmoney").text().replace(/\：(\d*)/, "："+$("#balance").val()));
				},
				buttons: {
					SURE: function(){
						var balance = Number($('#balance').val());
						var point = Number($('#point').val());
						if (point < 90000000 && point > balance) {
		...
		$('#searchLpBtn').click(function(){
			$('#lplist').ligerGrid({
				checkbox: false,
				rownumbers: true,
				columns: [{display: LPID, name: 'code', width:100},
						  {display: COL_A_COMPANYNAME, name: 'fullname'},
						  {display: COL_A_CREATE_TIME, name: 'create_time', width:87},
						  {display: COL_A_LAST_LOGIN_TIME, name: 'last_login_time', width:87},
						  {display: COL_A_HIGHLIGHT_DATE, name: 'highlightdate', width:87},
						  {display: COL_A_APPLY_VIEW_DATE, name: 'applydate', width:87},
						  {display: COL_A_BALANCE, name: 'balance', width: 90}], 
				dataAction: 'server', 
				pageSize: 20,
				width: "100%",
				columnWidth: 200,
				url: '/admin/admin_case/searchcasecreator',
				//url: '/admin/admin_lp/searchlplist',
				onDblClickRow: function(rowdata, rowindex, rowDomElement) {
					checkLp(rowdata);
					$('#dlg_searchLp').dialog('close');
				}
			});
			
			var manager = $("#lplist").ligerGetGridManager();
		...
		function checkLp(row) {
			if (!row) {
				alert(PLEASE_CHOISE_A_LP);
				return false;
			}
			$("#lp_code").val(row.code); 
			$("#lp_name").val(row.fullname); 
			$("#balance").val(row.balance); 
			$("#akb").val(''); 
			$("#currAkb").text($("#currAkb").text().replace(/\:(\d*)/, ":"+row.balance)); 
		}
	}
	...
	function initText() {
		...
		$("#lp_code").val(orgCase['lp_code']);
		$("#lp_name").val(orgCase['lp_name']);
