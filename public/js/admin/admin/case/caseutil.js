...
function viewCase(caseCode, btn, form, applyId) {
	form = form || $("#dynForm");
	if (form.length == 0) {
		window.open("/admin/admin_case/casedetail/caseCode/"+caseCode);
	} else {
		form.attr("action", "/admin/admin_case/casedetail/caseCode/"+caseCode);
		if (!!applyId)
...
function applyManager(caseCode, tab) {
	...
	if (!!tab)
		window.open("/admin/admin_applymgt/applymgt/caseCode/"+caseCode+"/t/"+tab);
	else
		window.open("/admin/admin_applymgt/applymgt/caseCode/"+caseCode);
}
...
function applyCase(row, manager, succFun) {
	...
	$.ligerDialog.confirm(SURE_APPLY_CHOOSE_CASE, TITLE_APPLY_CASE, function(e) {
		if (!e) return false;
		window.open("/admin/admin_caseapply/talentsearch/caseCode/"+row['code'])
	});
}
